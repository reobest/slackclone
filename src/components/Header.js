...
const Header = () => {
  ...
  const handleSearch = (value) => {
    if (value) {
      setMessege(messege.filter((m) => m.messege.toLowerCase().includes(value.toLowerCase())));
    } else {
  ...
  return (
    <Container>
      ...
      <HeaderSearch>
        ...
        <Input placeholder={`Search for messages`} onChange={(e) => handleSearch(e.target.value)} />
        <IoIosSearch />
      </HeaderSearch>
      <HeaderRight>
